	...
	for i in range(len(imdb.image_index)):
		if roidb[i]:
			...
			if not (imdb.name.startswith('coco')):
				...
				roidb[i]['depth'] = im.shape[0]       # 多加了一维
				roidb[i]['width'] = im.shape[1]
				roidb[i]['height'] = im.shape[2]
...
def combined_roidb(imdb_names, use_filpped ,training=True):
	...
	def get_roidb(imdb_name):
		...
		imdb,roidb = get_training_roidb(imdb)
		return imdb, roidb
	...
	# if training:
	imdb,roidb = filter_roidb(imdb,roidb)   # 对imdb也进行修改
