...
class star(imdb):
    def __init__(self, image_set, data_path):
        ...
        self.fitsfilelist=glob.glob(self._data_path+'/'+self._image_set +'data300-300/'+'*.fit*')          # data
        self.positionfilelist=glob.glob(self._data_path+'/'+self._image_set +'data300-300/'+'*.list')      # label
    ...
    def image_path_at(self, i):
        """
        Return the absolute path to image i in the image sequence.             (read data)
        """
    ...
    def gt_roidb(self):
        ...
        """
        cache_file = os.path.join(self.cache_path, self.name + '_gt_roidb.pkl')     # require update
        # 考虑加载cache_file文件
    ...
    def _load_star_txt(self,indx):         ##  change from _load_pascal_annotation(self,indx)
        ...
        # print(array_from_txt)
        if -1 in array_from_txt:
            print('There is no target in {}'.format(base_name))
            return {}
        objs_all_list = []
        if array_from_txt.size>4:
            for step, value_single_row in enumerate(array_from_txt):
                if 0 < array_from_txt[step, 1] < 1024 and 0 < array_from_txt[step, 2] < 1024:  ##  remove nagative coordinate(存在无目标的图像-1，-1)
                    objs_all_list.append(value_single_row)
        else:
            if 0 < array_from_txt[1] < 1024 and 0 < array_from_txt[2] < 1024:  ##  remove nagative coordinate(存在无目标的图像-1，-1)
        ...
        for ix in range(num_objs):      # if objs_all[ix][0] == 100:   # tip:The first is class
            padding = 15
            ...
            x1 = max(float(x1), 0)
            x2 = max(float(x2), 0)           # x2 = min(float(x2), 256)
            y1 = max(float(y1), 0)
            y2 = max(float(y2), 0)
            ishards[ix] = 0
            cls = self._class_to_ind['SKA']
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self._devkit_path, 'results', 'SKA_dectect', 'Main')
...
if __name__ == '__main__':
    d = star('train', '/home/lab30202/lwb/EP/detection_label')
    res = d.roidb
