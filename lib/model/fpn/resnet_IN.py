...
class SEBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,reduction=16):
        ...
        self.bn3 = self.norm_layer(planes * 2)
        self.leakyrelu = activate
        self.downsample = downsample
        self.stride = stride
        self.se = SELayer(planes*2, reduction)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, channel,num_classes=100):       # 可改？
        ...
        self.conv3 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.norm_layer = normal
        self.bn1 = self.norm_layer(channel)
        self.leakyrelu = activate
        ...
        self.layer4 = self._make_layer(block, channel*4, layers[3], stride=1)
...
def resnet18(channel,pretrained=False):
    """Constructs a ResNet-18 model.
    Args:
      pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    #model = ResNet(BasicBlock, [2, 2, 2, 2],channel)
    model = ResNet(Bottleneck, [2, 2, 2, 2], channel)
    # if pretrained:
    #   model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(channel,pretrained=False):
    """Constructs a ResNet-34 model.
    Args:
      pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3],channel)
    # if pretrained:
    #   model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(channel,pretrained=False):
    """Constructs a ResNet-50 model.
    Args:
      pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3],channel)
    #model = ResNet(SEBottleneck, [3, 4, 6, 3],channel)
...
class resnet(_FPN):
    def __init__(self, classes, num_layers=101,channel=3, pretrained=False, class_agnostic=False):
        self.model_path = '/home/lab30201/sdd/slc/SKAData/Faster_mulchannel/label_de/data/pretrained_model/SKA_detection.pth'   #  ？？？
        self.dout_base_model = channel    ## 256 -> 24
        self.pretrained = pretrained
        self.class_agnostic = class_agnostic
        self.channel = channel

        _FPN.__init__(self, classes,channel, class_agnostic)

    def _init_modules(self,channel):
        resnet = resnet50(channel)
        ...
        # Top layer
        self.RCNN_toplayer = nn.Conv2d(channel*8, channel, kernel_size=1, stride=1, padding=0)  # reduce channel

        # Smooth layers
        self.RCNN_smooth1 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)
        self.RCNN_smooth2 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)
        self.RCNN_smooth3 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)

        # Lateral layers
        self.RCNN_latlayer1 = nn.Conv2d(channel*6, channel, kernel_size=1, stride=1, padding=0)
        self.RCNN_latlayer2 = nn.Conv2d(channel*4, channel, kernel_size=1, stride=1, padding=0)
        self.RCNN_latlayer3 = nn.Conv2d(channel*2, channel, kernel_size=1, stride=1, padding=0)

        # ROI Pool feature downsampling
        self.RCNN_roi_feat_ds = nn.Conv2d(channel, channel, kernel_size=3, stride=2, padding=1)

        self.RCNN_top = nn.Sequential(
            nn.Conv2d(channel, channel*4, kernel_size=cfg.POOLING_SIZE, stride=cfg.POOLING_SIZE, padding=0),
            nn.LeakyReLU(),
        ...
        self.RCNN_cls_score = nn.Linear(channel*4, channel)
        if self.class_agnostic:
            self.RCNN_bbox_pred = nn.Linear(channel*4, 4)
        else:
            self.RCNN_bbox_pred = nn.Linear(channel*4, 4 * self.n_classes)
