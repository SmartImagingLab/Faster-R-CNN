...
class _RPN_FPN(nn.Module):
    ...
    def __init__(self, din):
        ...
        self.anchor_ratios = cfg.ANCHOR_RATIOS  #长宽比
        self.anchor_scales = cfg.ANCHOR_SCALES  #Anchor的基础尺寸的缩放比例
        self.feat_stride = cfg.FEAT_STRIDE[0]

        # define the convrelu layers processing input feature map
        self.RPN_Conv = nn.Conv2d(self.din, 128, 3, 1, 1, bias=True)

        # define bg/fg classifcation score layer
        # self.nc_score_out = len(self.anchor_scales) * len(self.anchor_ratios) * 2 # 2(bg/fg) * 9 (anchors)
        self.nc_score_out = 1 * len(self.anchor_ratios) * 2 # 2(bg/fg) * 3 (anchor ratios) * 1 (anchor scale)
        self.RPN_cls_score = nn.Conv2d(128, self.nc_score_out, 1, 1, 0) #获取单个像素每个anchor为bg/fg的得分

        # define anchor box offset prediction layer
        # self.nc_bbox_out = len(self.anchor_scales) * len(self.anchor_ratios) * 4 # 4(coords) * 9 (anchors)
        self.nc_bbox_out = 1 * len(self.anchor_ratios) * 4 # 4(coords) * 3 (anchors) * 1 (anchor scale)
        self.RPN_bbox_pred = nn.Conv2d(128, self.nc_bbox_out, 1, 1, 0)
    ...
    def forward(self, rpn_feature_maps, im_info, gt_boxes, num_boxes):        
        ...
        for i in range(n_feat_maps):
            ...
            rpn_cls_prob_reshape = F.softmax(rpn_cls_score_reshape,dim=1)   ## changed by lq
        ...
        if self.training:
            ...
            self.rpn_loss_cls = F.cross_entropy(rpn_cls_score, rpn_label)   # 交叉熵损失
            ...
            self.rpn_loss_box = _smooth_l1_loss(rpn_bbox_pred_alls, rpn_bbox_targets, rpn_bbox_inside_weights, 
                            rpn_bbox_outside_weights, sigma=3)              # Smooth L1 Loss
